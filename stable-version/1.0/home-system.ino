...
TaskHandle_t securityDeviceProcess;
...
#define WIFI_SSID "F<0>"
#define WIFI_PASSWORD "#Floor@0/Pass"
...
void securityDevice(/*void *pvParameters*/)
{
  // disableCore0WDT();
  ...
  if (latch)
  {
    ...
    informUsers(String("be aware someone trying to break into your house."), 0);
    pref.end();
  }
  delay(10);
}
        ...
        if (pref.getString(String(j).c_str(), "null").substring(0, pref.getString(String(j).c_str(), "null").indexOf("@")) == bot.messages[i].chat_id)
        {
          ...
          else if (bot.messages[i].text == "show options")
          {
            ...
            if (bot.messages[i].chat_id == admin)
              keyboard = "[[{\"text\" :\"open door\"},{\"text\" :\"close door\"}],[{\"text\" :\"Lights : ON\"},{\"text\" :\"Lights : OFF\"}],[{\"text\" :\"Security Mode : ON\"},{\"text\" :\"Security Mode : OFF\"}],[{\"text\" :\"Emergency Mode : ON\"},{\"text\" :\"Emergency Mode : OFF\"}],[{\"text\" :\"Get Users\"}]]";
            else
          ...
          {
            if (bot.messages[i].text != "Get Users" && bot.messages[i].text != "/start" && bot.messages[i].text.substring(0, 1) != "$" && bot.messages[i].text.substring(0, 1) != "#")
...
void setup()
{
  ...
  doorMotor.attach(doorPin, 500, 2400);
